    ...
    (define loop
      (make-define '(loop)
        (make-if test (list 'quote 'done)
          (make-begin (append other-exps (list '(loop)))))))
    (make-let '() (list loop '(loop)))))
  ...
  (let ((exprs (or-exprs expr)))
    (define (unfold-or exprs)
      (if (last-exp? exprs)
          ...
          (let ((first-expr (first-exp exprs))
                (rest-exprs (rest-exps exprs)))
            (make-let (list (list 'first first-expr))
                      (list (make-if 'first 'first
  ...
  (let ((message (case-message expr))
    ...
    (map (lambda (clause)
           (let ((cond-clause (first-cond-clause clause)))
             (if (list? cond-clause)
               (set-car! clause 
                         (make-eq? 'message cond-clause)))))
         clauses)
    (make-let (list (list 'message message))
